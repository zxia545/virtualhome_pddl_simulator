(define (domain virtualhome)
    ...
    (:action walk_into
        ...
        :effect (and
                (inside ?char ?room)
                (forall (?far_obj - object) 
                    (when (not (inside_room ?far_obj ?room)) 
                        (not (next_to ?char ?far_obj))
                    )
                )
        )
    ...
    (:action grab
        ...
        :precondition (and
                      (grabbable ?obj)
                      (next_to ?char ?obj)
                      (not (exists (?obj2 - object) 
                                (and (obj_inside ?obj ?obj2) (closed ?obj2))
                            )
                        )
                      (not (and (exists (?obj3 - object) 
                                    (holds_lh ?char ?obj3)
                                ) 
                                (exists (?obj4 - object) 
                                    (holds_rh ?char ?obj4)
        ...
        :effect (and
                (when (exists (?obj3 - object) 
                        (holds_lh ?char ?obj3)
                      ) 
                    (holds_rh ?char ?obj)
                )

                (when (exists (?obj4 - object) 
                        (holds_rh ?char ?obj4)
                      ) 
                    (holds_lh ?char ?obj)
                )
